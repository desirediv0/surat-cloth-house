    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      for (const variant of variants) {
        ...
        if (variant.sizeId) {
          ...
          if (weight) {
            sizeStr = `${weight.value}${weight.unit}`;
          }
        }
        ...
        if (
          ...
          const variantInfo = colorName + (sizeStr ? `-${sizeStr}` : "");
...
export const createProductVariant = asyncHandler(async (req, res, next) => {
  const { productId } = req.params;
  const { sku, colorId, sizeId, price, salePrice, quantity } = req.body;
  ...
  // Get size and color info for variant name
  let colorName = "";
  let sizeStr = "";
...
export const updateProductVariant = asyncHandler(async (req, res, next) => {
  ...
  const { sku, colorId, sizeId, price, salePrice, quantity, isActive } =
    req.body;
  ...
  if (
    ...
    let suffix = "";
    const newFlavorId = colorId !== undefined ? colorId : variant.colorId;
    const newWeightId = sizeId !== undefined ? sizeId : variant.sizeId;
