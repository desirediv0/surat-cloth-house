...
export const getMostViewedProducts = asyncHandler(async (req, res) => {
  ...
  const formattedProductViews = await Promise.all(
    productViews.map(async (view) => {
      ...
      // Get unique color and size counts
      const uniqueColors = new Set(
        product.variants.filter((v) => v.colorId).map((v) => v.colorId)
      );
      const uniqueSizes = new Set(
        product.variants.filter((v) => v.sizeId).map((v) => v.sizeId)
      );
      ...
      return {
        ...
        product: {
          ...
          variant: {
            ...
            colorImage: item.productVariant.color?.image
              ? getFileUrl(item.productVariant.color.image)
              : null,
